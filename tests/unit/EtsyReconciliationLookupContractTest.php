...
final class EtsyReconciliationLookupContractTest extends TestCase
{
 ...
 public function testLookupCannotClaimFailureFromAbsence():void{$s=(string)file_get_contents(dirname(__DIR__,2).'/includes/Listings/EtsyReconciliationLookupResponse.php');self::assertStringContainsString('listing_not_found_inconclusive',$s);self::assertStringContainsString('EtsyOperationLifecycle::UNKNOWN',$s);self::assertStringContainsString('EtsyOperationLifecycle::CONFIRMED_SUCCESS',$s);self::assertStringContainsString('operation_specific_evidence_required',$s);self::assertStringContainsString("operationType!=='CREATE_DRAFT'",$s);}
}
