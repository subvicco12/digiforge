...
final class EtsyReconciliationIdentityEvidenceContractTest extends TestCase
{
 ...
 public function testReconciliationServicePropagatesLookupIdentity():void{$s=(string)file_get_contents(dirname(__DIR__,2).'/includes/Listings/EtsyReconciliationService.php');self::assertStringContainsString("'lookup_reference'=>\$plan['lookup_reference']",$s);self::assertStringContainsString("'lookup_identity_available'=>\$plan['lookup_identity_available']",$s);}
}
 public function testRepositoryPersistsIdentityAtomicallyWithoutNetwork():void{$s=(string)file_get_contents(dirname(__DIR__,2).'/includes/Listings/EtsyOperationRepository.php');foreach(['recordReconciliationReference','reconciliation_reference_conflict','reconciliation_reference_state','EtsyOperationLifecycle::SENT','EtsyOperationLifecycle::UNKNOWN','idempotent_reconciliation_reference',"'reconciliation_reference'=>'"] as $n)self::assertStringContainsString($n,$s);foreach(['wp_remote_','curl_exec(','openapi.etsy'] as $n)self::assertStringNotContainsString($n,$s);}
