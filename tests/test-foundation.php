...
// Fast policy and structural checks that run without WordPress or Composer.
...
expect(str_contains($bootstrap, "DIGIFORGE_DB_VERSION = '6'"), 'database schema version is current');
...
foreach (['automation_armed', "self::get('stop_all', true)", 'Config::valid_value', 'safety_locked'] as $guard) {
    expect(str_contains($settings, $guard), "$guard fail-closed guard exists");
}
