...
expect((bool) preg_match('/^\\s*\\*\\s*Version:\\s*1\\.0\\.21\\s*$/m', $bootstrap), 'plugin header release version is current');
expect((bool) preg_match("/const\\s+DIGIFORGE_VERSION\\s*=\\s*'1\\.0\\.21'\\s*;/", $bootstrap), 'runtime release version is current');
