...
final class ExecutionFailureRepositoryTest extends WP_UnitTestCase
{
 ...
 private function failure(string $hash):array{
  return ['state'=>'EXECUTION_FAILED_RECORDED','failure_hash'=>$hash,'failure'=>[
   ...
   'adapter_status'=>'FAILED','failure_category'=>'adapter_error','failure_code'=>'provider_timeout','nonce_consumed'=>true,'retry_permitted'=>false,
  ]];
 }
 public function testExactReplayReturnsSameFailure():void{
  ...
  self::assertIsArray($a);self::assertSame('adapter_error',$a['failure_category']);self::assertSame('provider_timeout',$a['failure_code']);$b=DigiForge\POD\ExecutionFailureRepository::save($this->failure($h));
