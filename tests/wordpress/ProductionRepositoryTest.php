...
final class ProductionRepositoryTest extends WP_UnitTestCase
{
    ...
    private function assertNotWpError(mixed $value, string $context): void
    {
    ...
    public function testReleaseReadinessRequiresRelationshipsQaAndHumanApproval(): void
    {
        ...
        $this->assertNotWpError($spec,'create spec');
        ...
        $this->assertNotWpError($replaySeed,'create replay seed');
        ...
        $this->assertNotWpError($replay,'replay spec');
        ...
        $this->assertNotWpError($plan,'create plan');
        ...
        $this->assertNotWpError($intent,'create intent');
        ...
        $this->assertNotWpError($revision,'add revision');
        ...
        $this->assertNotWpError($qaPassed,'add QA');

        $this->assertNotWpError($this->repo->transition('revision',(int)$revision['id'],'QA_PASSED'),'revision QA_PASSED');
        $this->assertNotWpError($this->repo->transition('revision',(int)$revision['id'],'APPROVED'),'revision APPROVED');
        $this->assertNotWpError($this->repo->transition('plan',(int)$plan['id'],'VALIDATED'),'plan VALIDATED');
        $this->assertNotWpError($this->repo->transition('plan',(int)$plan['id'],'REVIEW_REQUIRED'),'plan REVIEW_REQUIRED');
        $this->assertNotWpError($this->repo->transition('plan',(int)$plan['id'],'APPROVED'),'plan APPROVED');
        ...
        $this->assertNotWpError($bundle,'create bundle');
        $this->assertNotWpError($this->repo->transition('bundle',(int)$bundle['id'],'VALIDATED'),'bundle VALIDATED');
        $this->assertNotWpError($this->repo->transition('bundle',(int)$bundle['id'],'REVIEW_REQUIRED'),'bundle REVIEW_REQUIRED');

        $beforeApproval=$this->repo->validateBundle((int)$bundle['id']);
        $this->assertNotWpError($beforeApproval,'validate before approval');
        self::assertFalse($beforeApproval['readiness']['ready']);
        self::assertSame('REVIEW_REQUIRED',$beforeApproval['state']);

        $this->assertNotWpError($this->repo->transition('bundle',(int)$bundle['id'],'APPROVED'),'bundle APPROVED');
        ...
        $this->assertNotWpError($ready,'validate ready');
        ...
        $this->assertNotWpError($again,'validate deterministically');
    ...
    public function testMissingQaAndCrossProductLinksFailClosed(): void
    {
        ...
        $this->assertNotWpError($spec,'create secondary spec');
        ...
        $this->assertNotWpError($plan,'create secondary plan');
        ...
        $this->assertNotWpError($revision,'add secondary revision');
        $this->assertNotWpError($this->repo->transition('revision',(int)$revision['id'],'QA_PASSED'),'secondary revision QA_PASSED');
        $this->assertNotWpError($this->repo->transition('revision',(int)$revision['id'],'APPROVED'),'secondary revision APPROVED');
        foreach(['VALIDATED','REVIEW_REQUIRED','APPROVED'] as $state)$this->assertNotWpError($this->repo->transition('plan',(int)$plan['id'],$state),'secondary plan '.$state);
        $bundle=$this->repo->createBundle(['production_plan_id'=>$plan['id'],'bundle_key'=>'no-qa','version_label'=>'1'],'bundle-2-'.wp_rand(100000,999999));
        $this->assertNotWpError($bundle,'create no-QA bundle');
        foreach(['VALIDATED','REVIEW_REQUIRED','APPROVED'] as $state)$this->assertNotWpError($this->repo->transition('bundle',(int)$bundle['id'],$state),'secondary bundle '.$state);
        $blocked=$this->repo->validateBundle((int)$bundle['id']);
        $this->assertNotWpError($blocked,'validate missing-QA bundle');
        ...
        $this->assertNotWpError($otherSpec,'create cross-product spec');
