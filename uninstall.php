...
global $wpdb;
foreach (['settings','audit_log','jobs','idempotency','health_events','opportunities','product_families','products','product_versions','digital_products','digital_files','digital_file_versions','digital_packages','digital_previews','digital_templates','digital_licenses','digital_download_checks','integration_secrets','integrations','research_candidate_evidence','research_reviews','research_candidates','research_evidence','research_observations','research_sources','ai_reviews','ai_usage','ai_outputs','ai_runs','ai_prompt_versions','ai_prompts','ai_models','ai_tasks'] as $table) { $wpdb->query('DROP TABLE IF EXISTS `' . esc_sql($wpdb->prefix . 'digiforge_' . $table) . '`'); }
