...
final class Admin
{
    ...
    public function render(): void
    {
        ...
        $snapshot = $this->recentListingStates();
        $summary = AdminStateSummary::summarize($snapshot['rows']);
        ...
        echo '<tr><th>'.esc_html__('Snapshot query status','digiforge').'</th><td><strong>'.esc_html($snapshot['query_ok'] ? 'PASS' : 'FAILED').'</strong></td></tr>';
    ...
    /** @return array{rows:array<int,array{state:mixed}>,query_ok:bool} */
    private function recentListingStates(): array
    {
        ...
        if ($rows === null || (string) $wpdb->last_error !== '') {
            return ['rows' => [], 'query_ok' => false];
        }
        return ['rows' => is_array($rows) ? $rows : [], 'query_ok' => true];
    }
}
