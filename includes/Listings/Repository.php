...
final class Repository
{
    ...
    {
        $listing=$this->find(Tables::listings(),$listingId);if(!is_array($listing))return $this->error('not_found','Listing not found.',404);global $wpdb;$pv=(int)$listing['product_version_id'];$seo=(int)$wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM '.Tables::listing_seo().' WHERE listing_id=%d',$listingId));$mediaRows=$wpdb->get_results($wpdb->prepare('SELECT * FROM '.Tables::listing_media().' WHERE listing_id=%d ORDER BY position_index ASC,id ASC',$listingId),ARRAY_A)?:[];
        $digital=(int)$wpdb->get_var($wpdb->prepare('SELECT COUNT(*) FROM '.Tables::digital_products().' WHERE product_version_id=%d',$pv))>0;$podMappings=(int)$wpdb->get_var($wpdb->prepare("SELECT COUNT(*) FROM ".Tables::pod_mappings()." WHERE product_version_id=%d AND environment=%s AND state='APPROVED'",$pv,(string)$listing['environment']));$podBindings=$wpdb->get_results($wpdb->prepare('SELECT * FROM '.Tables::listing_pod_bindings().' WHERE listing_id=%d',$listingId),ARRAY_A)?:[];$pod=$podMappings>0||$podBindings!==[];
