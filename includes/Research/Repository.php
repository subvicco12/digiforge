...
final class Repository {
    ...
    private function replayCompatible(array $existing,array $data):bool{foreach($data as $key=>$value){if(in_array($key,['id','idempotency_key','created_at','updated_at','review_status','opportunity_id'],true))continue;if(!array_key_exists($key,$existing))continue;if(!$this->sameReplayValue($key,$existing[$key],$value))return false;}return true;}
