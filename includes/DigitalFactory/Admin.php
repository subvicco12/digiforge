...
final class Admin {
    private const PAGES=['digital_product'=>'Digital Products','digital_file'=>'Digital Files','digital_file_version'=>'Digital File Versions','digital_package'=>'Digital Packages','digital_preview'=>'Digital Previews','digital_template'=>'Digital Templates','digital_license'=>'Digital Licenses','digital_download_check'=>'Digital QA / Download Checks'];
    ...
    public function render(string $type,string $label): void {
        ...
        $operationalField=$type==='digital_package'?'generation_status':(in_array($type,['digital_file','digital_file_version','digital_preview','digital_template'],true)?'status':'');
        ...
        <?php if(is_array($qaSummary)): ?><p><strong><?php esc_html_e('Current page QA snapshot:','digiforge'); ?></strong> <?php echo esc_html(sprintf(__('sampled %1$d; attention %2$d; invalid/unknown %3$d; readiness inferred: NO; external actions performed: NO','digiforge'),(int)$qaSummary['total'],(int)($qaSummary['attention_total']??0),(int)$qaSummary['invalid'])); ?></p><?php endif; ?>
        ...
        <?php if(!$queryOk): ?><tr><td colspan="4"><?php esc_html_e('Records unavailable because the snapshot query failed.','digiforge'); ?></td></tr><?php elseif($items===[]): ?><tr><td colspan="4"><?php esc_html_e('No records found.','digiforge'); ?></td></tr><?php endif; foreach($items as $item): ?><tr><td><?php echo esc_html((string)$item['id']); ?></td><td><?php echo esc_html((string)($item['name']??$item['version_label']??$item['check_type']??'')); ?></td><td><?php echo esc_html((string)($item['state']??$item['status']??$item['generation_status']??$item['validation_result']??'')); ?></td><td><?php echo esc_html((string)$item['updated_at']); ?></td></tr><?php endforeach; ?>
        </tbody></table>
