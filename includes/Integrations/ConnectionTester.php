...
final class ConnectionTester {
    ...
    public function test(int $integrationId): array|\WP_Error {
        ...
        return match ((string) ($integration['provider'] ?? '')) {
            ...
            default => new \WP_Error(
                ...
                __('This custom provider does not yet have a safe read-only connection test profile.', 'digiforge'),
                ['status' => 400]
            ),
        };
    }
    ...
    private function testPrintify(int $integrationId): array|\WP_Error {
        ...
        if (is_wp_error($token)) {
            return new \WP_Error('printify_credential_missing', __('Store a Printify personal access token before testing the connection.', 'digiforge'), ['status' => 409]);
        }
        $response = $this->get(self::PRINTIFY_SHOPS_URL, [
            'Authorization' => 'Bearer ' . $token,
            'Accept' => 'application/json',
        ]);
        unset($token);
        $checked = $this->checkHttp('printify', $integrationId, $response);
        if (is_wp_error($checked)) { return $checked; }

        $decoded = json_decode((string) wp_remote_retrieve_body($response), true);
        if (! is_array($decoded)) { return $this->invalidResponse('printify', $integrationId); }
        $shops = [];
    ...
    private function testEtsy(int $integrationId): array|\WP_Error {
        ...
        $shared = $this->credential($integrationId, 'shared_secret');
        if (is_wp_error($keystring) || is_wp_error($shared)) {
            return new \WP_Error('etsy_app_credentials_missing', __('Store both Etsy keystring and shared_secret before testing the connection.', 'digiforge'), ['status' => 409]);
        }
        $apiKey = $keystring . ':' . $shared;
        unset($keystring, $shared);
        $ping = $this->get(self::ETSY_PING_URL, ['x-api-key' => $apiKey, 'Accept' => 'application/json']);
        $checked = $this->checkHttp('etsy', $integrationId, $ping, false);
        if (is_wp_error($checked)) { return $checked; }
        ...
        $userId = isset($parts[0]) ? absint($parts[0]) : 0;
        if ($userId <= 0) {
            return new \WP_Error('etsy_access_token_invalid', __('The stored Etsy access token does not contain a valid user identifier.', 'digiforge'), ['status' => 409]);
        }
        $userResponse = $this->get(self::ETSY_USER_BASE . $userId, [
            'x-api-key' => $apiKey,
            'Authorization' => 'Bearer ' . $access,
            'Accept' => 'application/json',
        ]);
        unset($access, $apiKey);
        $checked = $this->checkHttp('etsy', $integrationId, $userResponse);
        if (is_wp_error($checked)) { return $checked; }
        $user = json_decode((string) wp_remote_retrieve_body($userResponse), true);
        if (! is_array($user)) { return $this->invalidResponse('etsy', $integrationId); }
    ...
    private function testGelato(int $integrationId): array|\WP_Error {
        ...
        if (is_wp_error($apiKey)) {
            return new \WP_Error('gelato_credential_missing', __('Store the Gelato api_key before testing the connection.', 'digiforge'), ['status' => 409]);
        }
        $response = $this->get(self::GELATO_CATALOGS_URL, ['X-API-KEY' => $apiKey, 'Accept' => 'application/json']);
        unset($apiKey);
        $checked = $this->checkHttp('gelato', $integrationId, $response);
        if (is_wp_error($checked)) { return $checked; }
        $decoded = json_decode((string) wp_remote_retrieve_body($response), true);
        if (! is_array($decoded)) { return $this->invalidResponse('gelato', $integrationId); }
    ...
    private function testAi(int $integrationId, array $integration): array|\WP_Error {
        ...
        $vendor = sanitize_key((string) ($config['vendor'] ?? ''));
        if ($vendor === '') {
            return new \WP_Error('ai_vendor_missing', __('Set the AI connector vendor before testing it. OpenAI is supported by the current tester.', 'digiforge'), ['status' => 409]);
        }
        if ($vendor !== 'openai') {
            return new \WP_Error('ai_vendor_not_supported', sprintf(__('No audited read-only connection test is available yet for AI vendor "%s".', 'digiforge'), $vendor), ['status' => 400]);
        }
        $apiKey = $this->credential($integrationId, 'api_key');
        if (is_wp_error($apiKey)) {
            return new \WP_Error('ai_credential_missing', __('Store the AI provider api_key before testing the connection.', 'digiforge'), ['status' => 409]);
        }
        $response = $this->get(self::OPENAI_MODELS_URL, ['Authorization' => 'Bearer ' . $apiKey, 'Accept' => 'application/json']);
        unset($apiKey);
        $checked = $this->checkHttp('ai', $integrationId, $response);
        if (is_wp_error($checked)) { return $checked; }
        $decoded = json_decode((string) wp_remote_retrieve_body($response), true);
        if (! is_array($decoded)) { return $this->invalidResponse('ai', $integrationId); }
        $modelCount = is_array($decoded['data'] ?? null) ? count($decoded['data']) : 0;
        $details = ['http_status' => 200, 'vendor' => 'openai', 'model_count' => $modelCount];
        $this->record($integrationId, true, $details, true);
    ...
    private function get(string $url, array $headers): array|\WP_Error {
        return wp_remote_get($url, [
            'timeout' => 12,
            'redirection' => 0,
            'reject_unsafe_urls' => true,
            'sslverify' => true,
            'headers' => $headers + ['User-Agent' => 'DigiForge WordPress Integration Tester'],
        ]);
    }
    ...
    private function checkHttp(string $provider, int $integrationId, array|\WP_Error $response, bool $recordFailure = true): true|\WP_Error {
        if (is_wp_error($response)) {
            if ($recordFailure) { $this->record($integrationId, false, ['reason' => 'transport_error'], false); }
            Logger::audit('integration_connection_test_failed', ['provider' => $provider, 'reason' => 'transport_error'], 'integration', (string) $integrationId);
            return new \WP_Error($provider . '_transport_error', sprintf(__('%s could not be reached from this WordPress server.', 'digiforge'), ucfirst($provider)), ['status' => 502]);
        }
        $status = (int) wp_remote_retrieve_response_code($response);
        if ($status === 401 || $status === 403) {
            if ($recordFailure) { $this->record($integrationId, false, ['http_status' => $status, 'reason' => 'authentication_failed'], false); }
            Logger::audit('integration_connection_test_failed', ['provider' => $provider, 'http_status' => $status, 'reason' => 'authentication_failed'], 'integration', (string) $integrationId);
            return new \WP_Error($provider . '_authentication_failed', sprintf(__('%s rejected the stored credential or its permissions.', 'digiforge'), ucfirst($provider)), ['status' => 401]);
        }
        if ($status < 200 || $status >= 300) {
            if ($recordFailure) { $this->record($integrationId, false, ['http_status' => $status, 'reason' => 'unexpected_response'], false); }
            Logger::audit('integration_connection_test_failed', ['provider' => $provider, 'http_status' => $status, 'reason' => 'unexpected_response'], 'integration', (string) $integrationId);
            return new \WP_Error($provider . '_unexpected_response', sprintf(__('%1$s returned HTTP %2$d during the read-only connection test.', 'digiforge'), ucfirst($provider), $status), ['status' => 502]);
        }
        return true;
    }

    private function invalidResponse(string $provider, int $integrationId): \WP_Error {
        $this->record($integrationId, false, ['reason' => 'invalid_json'], false);
        return new \WP_Error($provider . '_invalid_response', sprintf(__('%s returned an unreadable response.', 'digiforge'), ucfirst($provider)), ['status' => 502]);
    }

    private function firstCredential(int $integrationId, array $names): string|\WP_Error {
        foreach ($names as $name) {
            $value = $this->credential($integrationId, (string) $name);
            if (! is_wp_error($value) && $value !== '') { return $value; }
        }
    ...
    private function record(int $integrationId, bool $success, array $details, bool $markConfigured): void {
        ...
        $config['_connection_test'] = ['ok' => $success, 'checked_at' => current_time('mysql', true), 'details' => $details];
        ...
        $wpdb->update(Tables::integrations(), ['status' => $status, 'config' => $encoded, 'updated_at' => current_time('mysql', true)], ['id' => $integrationId]);
    }
}
