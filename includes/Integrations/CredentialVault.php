...
final class CredentialVault {
    ...
    public static function encrypt(string $plaintext): string {
        ...
        if (function_exists('sodium_crypto_aead_xchacha20poly1305_ietf_encrypt')) {
            ...
            $cipher = sodium_crypto_aead_xchacha20poly1305_ietf_encrypt($plaintext, 'digiforge', $nonce, $key);
        ...
        $cipher = openssl_encrypt($plaintext, 'aes-256-gcm', $key, OPENSSL_RAW_DATA, $iv, $tag, 'digiforge');
    ...
    public static function decrypt(string $payload): string {
        ...
        if ($driver === 'sodium') {
            ...
            $plain = sodium_crypto_aead_xchacha20poly1305_ietf_decrypt(substr($raw, $n), 'digiforge', substr($raw, 0, $n), $key);
        } elseif ($driver === 'openssl') {
            $plain = openssl_decrypt(substr($raw, 28), 'aes-256-gcm', $key, OPENSSL_RAW_DATA, substr($raw, 0, 12), substr($raw, 12, 16), 'digiforge');
    ...
    public static function fingerprint(string $plaintext): string {
        return substr(hash_hmac('sha256', $plaintext, self::derive('fingerprint')), 0, 16);
    }
}
