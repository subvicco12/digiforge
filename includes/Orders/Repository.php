...
final class Repository
{
    ...
    public function transition(string $entity,int $id,string $to): array|WP_Error
    {
        ...
        if($entity==='intent'&&in_array($to,['READY_FOR_REVIEW','APPROVED_INTENT'],true)){$plan=$this->find(Tables::fulfillment_plans(),(int)($row['fulfillment_plan_id']??0));$boundHash=strtolower(trim((string)($row['fulfillment_plan_payload_hash']??'')));$currentHash=is_array($plan)?strtolower(trim((string)($plan['payload_hash']??''))):'';if(!is_array($plan)||(int)$plan['order_id']!==(int)$row['order_id']||(string)$plan['environment']!==(string)$row['environment']||(string)$plan['state']!=='APPROVED'||!preg_match('/^[a-f0-9]{64}$/',$boundHash)||!preg_match('/^[a-f0-9]{64}$/',$currentHash)||!hash_equals($boundHash,$currentHash))return $this->error('plan_evidence','Fulfillment intent plan evidence is stale or invalid; rebuild the intent from the approved plan.',409);}
