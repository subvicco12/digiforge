...
final class Repository
{
    ...
    public function reviewPersonalization(int $id,string $status): array|WP_Error
    {
        ...
        global $wpdb;$ok=$wpdb->update(Tables::personalization_submissions(),['review_status'=>$status,'reviewed_by'=>get_current_user_id(),'reviewed_at'=>$this->now(),'updated_at'=>$this->now()],['id'=>$id,'review_status'=>'UNREVIEWED']);if($ok!==1)return $this->error('transition_conflict','Personalization review changed concurrently or update failed.',409);Logger::audit('personalization_reviewed',get_current_user_id(),'personalization_submission',(string)$id,['status'=>$status]);return $this->find(Tables::personalization_submissions(),$id)?:[];
    }
    ...
    {
        $table=match($entity){'order'=>Tables::orders(),'plan'=>Tables::fulfillment_plans(),'intent'=>Tables::fulfillment_intents(),default=>''};if($table==='')return $this->error('validation','Unknown lifecycle entity.');$row=$this->find($table,$id);$to=strtoupper(sanitize_key($to));if(!is_array($row))return $this->error('not_found','Order record not found.',404);if((string)$row['state']===$to)return $row+['idempotent_transition'=>true];if(!Lifecycle::can($entity,(string)$row['state'],$to))return $this->error('invalid_transition','Lifecycle transition is not permitted.',409);if(in_array($to,['APPROVED','APPROVED_INTENT'],true)&&get_current_user_id()<1)return $this->error('reviewer_required','Authenticated human reviewer required.',403);
        ...
        global $wpdb;$data=['state'=>$to,'updated_at'=>$this->now()];if(($entity==='order'||$entity==='plan')&&$to==='APPROVED'){$data['approved_by']=get_current_user_id();$data['approved_at']=$this->now();}if($wpdb->update($table,$data,['id'=>$id,'state'=>(string)$row['state']])!==1)return $this->error('transition_conflict','State changed concurrently or update failed.',409);Logger::audit('order_state_changed',get_current_user_id(),$entity,(string)$id,['from'=>$row['state'],'to'=>$to]);return $this->find($table,$id)?:[];
    }
