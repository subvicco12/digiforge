...
final class LocalAssetProducer
{
    ...
    public function write(int $productVersionId, string $filename, string $format, $content): array|WP_Error
    {
        ...
        if ($bytes === '' || strlen($bytes) > self::MAX_ASSET_BYTES) {
            return $this->error('invalid_asset_size', 'Generated asset is empty or too large.');
        }
        ...
        if (file_put_contents($temp, $bytes, LOCK_EX) !== strlen($bytes)) {
            @unlink($temp);
            return $this->error('asset_write', 'Unable to write generated asset.', 500);
        }
        if (! @rename($temp, $path)) {
            @unlink($temp);
    ...
    public function package(int $productVersionId, array $assets, string $filename = 'customer-package.zip'): array|WP_Error
    {
        ...
        foreach ($assets as $asset) {
            ...
            if ($source === '' || $name === '' || ! is_file($source) || ! $zip->addFile($source, $name)) {
                $zip->close();
                @unlink($path);
                return $this->error('zip_asset', 'Unable to add a required product asset to ZIP.', 500);
            }
        }
        $zip->close();
        return [
    ...
    {
        if ($format === 'json') {
            $encoded = wp_json_encode($content, JSON_UNESCAPED_SLASHES | JSON_UNESCAPED_UNICODE | JSON_PRETTY_PRINT);
            return is_string($encoded) ? $encoded : $this->error('json_encode', 'Unable to encode JSON asset.');
        }
    ...
    private function pdf($content): string|WP_Error
    {
        ...
        if ($pages === []) {
            return $this->error('invalid_pdf_content', 'PDF requires at least one text page.');
        }
        $objects = [];
        $pageIds = [];
        $fontId = 3;
    ...
    {
        return match ($format) {
            'pdf' => 'application/pdf', 'svg' => 'image/svg+xml', 'html' => 'text/html',
            'csv' => 'text/csv', 'json' => 'application/json', default => 'text/plain',
        };
    }
