...
final class Logger {
    private const REDACT = ['password','secret','token','authorization','api_key','apikey','credential'];
    ...
    public static function redact(array $context): array { foreach ($context as $key => $value) { if (in_array(strtolower((string) $key), self::REDACT, true)) { $context[$key] = '[REDACTED]'; } elseif (is_array($value)) { $context[$key] = self::redact($value); } } return $context; }
}
