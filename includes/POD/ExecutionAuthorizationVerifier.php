...
final class ExecutionAuthorizationVerifier
{
 ...
 public static function verify(array $record,string $requiredAction,string $expectedEvidenceHash,int $now,callable $nonceUnused):true|WP_Error
 {
  ...
  if(!in_array($requiredAction,['ETSY_DRAFT_CREATE','ETSY_DRAFT_UPDATE','ETSY_DRAFT_INVENTORY','ETSY_DRAFT_IMAGE','PROVIDER_ORDER_SUBMIT','PROVIDER_PRODUCTION_AUTHORIZE'],true)||($a['action']??'')!==$requiredAction)
