...
final class PrintifyControlledTransport{
 ...
 public function execute(array $permit,array $payload):array|WP_Error{
  ...
  return (new PrintifyScopedCredentialRetriever())->consume((int)$request['integration_id'],static function(string $token)use($sender,$request,$permit){
   ...
   if(is_wp_error($response)){$response->add_data(['network_request_attempted'=>true,'request_fingerprint'=>$request['request_fingerprint']]);return $response;}
   ...
   return ['status'=>'UNKNOWN','action'=>$permit['action'],'authorization_hash'=>$permit['authorization_hash'],'evidence_hash'=>$permit['evidence_hash'],'failure_category'=>'AMBIGUOUS_HTTP','failure_code'=>'http_'.$status];
  });
 }
}
