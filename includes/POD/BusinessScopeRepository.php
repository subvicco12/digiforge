...
final class BusinessScopeRepository
{
    ...
    public function createMapping(array $input,?string $idempotencyKey=null,bool $manageTransaction=true): array|WP_Error
    {
        ...
        try{
            ...
            $owner=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.Tables::pod_business_mappings().' WHERE product_version_id=%d AND provider_mapping_id=%d LIMIT 1 FOR UPDATE',$productVersionId,$providerMappingId),ARRAY_A);if(is_array($owner)){if($key!==null&&hash_equals((string)($owner['idempotency_key']??''),$key)&&hash_equals((string)($owner['request_fingerprint']??''),$fingerprint)){if($manageTransaction)$wpdb->query('COMMIT');return $owner;}if($manageTransaction)$wpdb->query('ROLLBACK');return new WP_Error('digiforge_scope_ownership_conflict','Product/provider mapping already has business ownership.',['status'=>409]);}
