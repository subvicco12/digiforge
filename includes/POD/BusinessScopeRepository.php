...
final class BusinessScopeRepository
{
    ...
    public function createMapping(array $input,?string $idempotencyKey=null): array|WP_Error
    {
        ...
        try{
            ...
            $provider=$wpdb->get_row($wpdb->prepare('SELECT product_version_id FROM '.Tables::pod_mappings().' WHERE id=%d FOR UPDATE',$providerMappingId),ARRAY_A);if(!is_array($provider)||(int)$provider['product_version_id']!==$productVersionId){$wpdb->query('ROLLBACK');return new WP_Error('digiforge_scope_relationship','Provider mapping must exist and match the product version.',['status'=>409]);}
            $owner=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.Tables::pod_business_mappings().' WHERE product_version_id=%d AND provider_mapping_id=%d LIMIT 1 FOR UPDATE',$productVersionId,$providerMappingId),ARRAY_A);if(is_array($owner)){$wpdb->query('ROLLBACK');return new WP_Error('digiforge_scope_ownership_conflict','Product/provider mapping already has business ownership.',['status'=>409]);}
