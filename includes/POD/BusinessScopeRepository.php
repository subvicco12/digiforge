...
final class BusinessScopeRepository
{
    public function replay(array $input, ?string $idempotencyKey): array|WP_Error|null
    {
        ...
        $existing=$wpdb->get_row($wpdb->prepare('SELECT * FROM '.Tables::pod_business_mappings().' WHERE idempotency_key=%s LIMIT 1',$key),ARRAY_A);
        if(!is_array($existing)) return null;
        $fingerprint=$this->requestFingerprint($input);
        if(is_wp_error($fingerprint)) return $fingerprint;
        return hash_equals((string)($existing['request_fingerprint']??''),$fingerprint)
            ? $existing
            : new WP_Error('digiforge_idempotency_conflict','Idempotency key is already bound to a different business scope or mapping.',['status'=>409]);
    }

    public function createMapping(array $input, ?string $idempotencyKey=null): array|WP_Error
    {
        global $wpdb;
        $replay=$this->replay($input,$idempotencyKey); if($replay!==null) return $replay;
        $productVersionId=absint($input['product_version_id']??0);$providerMappingId=absint($input['provider_mapping_id']??0);
        if($productVersionId<1||$providerMappingId<1) return new WP_Error('digiforge_scope_validation','product_version_id and provider_mapping_id are required.',['status'=>400]);
        $key=$idempotencyKey===null?null:sanitize_text_field($idempotencyKey);if($key==='')$key=null;
        $fingerprint=$this->requestFingerprint($input);if(is_wp_error($fingerprint))return $fingerprint;
        $wpdb->query('START TRANSACTION');
        try{
            try{$scope=BusinessScope::resolveConfigured($input);}catch(\InvalidArgumentException $e){$wpdb->query('ROLLBACK');return new WP_Error('digiforge_scope_validation',$e->getMessage(),['status'=>409]);}
            ...
            if(is_array($owner)){$wpdb->query('ROLLBACK');return new WP_Error('digiforge_scope_ownership_conflict','Product/provider mapping already has business ownership.',['status'=>409]);}
            $now=current_time('mysql',true);
            $inserted=$wpdb->insert(Tables::pod_business_mappings(),['business_id'=>$scope['business_id'],'store_id'=>$scope['store_id'],'product_program_id'=>$scope['product_program_id'],'product_version_id'=>$productVersionId,'provider_mapping_id'=>$providerMappingId,'state'=>'DRAFT','request_fingerprint'=>$fingerprint,'idempotency_key'=>$key,'created_by'=>get_current_user_id(),'created_at'=>$now,'updated_at'=>$now]);
    ...
    {
        $productVersionId=absint($input['product_version_id']??0);$providerMappingId=absint($input['provider_mapping_id']??0);
        $business=sanitize_key((string)($input['business_id']??''));$store=sanitize_key((string)($input['store_id']??''));$program=strtoupper(trim((string)($input['product_program']??'')));
        ...
        $payload=wp_json_encode(['business_ref'=>$business,'store_ref'=>$store,'product_program'=>$program,'product_version_id'=>$productVersionId,'provider_mapping_id'=>$providerMappingId]);
        if(!is_string($payload)) return new WP_Error('digiforge_scope_validation','Unable to fingerprint scope request.',['status'=>400]);
