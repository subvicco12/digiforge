...
final class PrintifyMutationRequest{
 public static function prepare(array $permit,array $payload):array|WP_Error{
  ...
  if($action==='PROVIDER_ORDER_SUBMIT'){
   ...
   $order=preg_replace('/[^A-Za-z0-9_-]/','',(string)($payload['order_id']??''));if($order==='')return self::e('order_id','Provider order identity is required.');
  ...
  $material=['action'=>$action,'integration_id'=>$integration,'shop_id'=>$shop,'endpoint'=>$endpoint,'body'=>$body];$json=wp_json_encode(self::canon($material));
  if(!is_string($json)||strlen($json)>262144)return self::e('payload','Printify mutation evidence is invalid or too large.');
  return ['state'=>'PRINTIFY_MUTATION_REQUEST_PREPARED','provider'=>'printify','api_version'=>'v1','v2_preference_preserved'=>true,'v1_required_for_operation'=>true,'action'=>$action,'integration_id'=>$integration,'shop_id'=>$shop,'method'=>'POST','endpoint'=>$endpoint,'body'=>$body,'existing_external_reference'=>$external,'request_fingerprint'=>hash('sha256',$json),'network_request_permitted'=>false,'network_request_attempted'=>false,'external_execution_performed'=>false];
 }
