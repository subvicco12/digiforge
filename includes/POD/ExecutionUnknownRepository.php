...
final class ExecutionUnknownRepository
{
 ...
 public static function save(array $record):array|WP_Error
 {
  ...
  $r=$record['unknown'];$hash=(string)($record['unknown_hash']??'');$auth=(string)($r['authorization_hash']??'');
  if(!preg_match('/^[a-f0-9]{64}$/',$hash)||!preg_match('/^[a-f0-9]{64}$/',$auth)||($r['retry_permitted']??null)!==false||($r['reconciliation_required']??null)!==true)
  ...
  $row=['action'=>(string)$r['action'],'evidence_hash'=>(string)$r['evidence_hash'],'authorization_hash'=>$auth,'nonce_hash'=>(string)$r['authorization_nonce_hash'],'failure_category'=>sanitize_key((string)$r['failure_category']),'failure_code'=>sanitize_key((string)$r['failure_code']),'executed_by'=>(int)$r['executed_by'],'recorded_at'=>gmdate('Y-m-d H:i:s',(int)$r['recorded_at']),'unknown_hash'=>$hash,'created_at'=>current_time('mysql',true)];
