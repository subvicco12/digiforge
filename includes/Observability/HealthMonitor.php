...
{
    /** @return array{status:string, automation_locked:bool, schema:array{current:int, expected:int}, audit:array{status:string, last_failure:?array<string, string>}, queue:array{status:string, expired_leases:int, dead_letters:int}} */
    public function snapshot(): array
    {
        global $wpdb;
        $currentSchema = (int) get_option('digiforge_db_schema_version', 0);
        $expiredLeases = (int) $wpdb->get_var(
            $wpdb->prepare(
                'SELECT COUNT(*) FROM ' . Tables::jobs() . " WHERE lease_expires_at IS NOT NULL AND lease_expires_at < %s AND state = 'RUNNING'",
                current_time('mysql', true)
            )
        );
        $deadLetters = (int) $wpdb->get_var("SELECT COUNT(*) FROM " . Tables::jobs() . " WHERE state = 'DEAD_LETTER'");
        ...
        $queueStatus = $expiredLeases > 0 ? HealthStatus::DEGRADED : HealthStatus::HEALTHY;
        ...
        return [
            ...
            'queue' => ['status' => $queueStatus, 'expired_leases' => $expiredLeases, 'dead_letters' => $deadLetters],
        ];
    }
}
