...
final class IntegrationsController {
    ...
    private function mutate(\WP_REST_Request $request, string $operation, callable $callback, int $successStatus = 200, bool $allowBodyIdempotencyKey = false): \WP_REST_Response|\WP_Error {
        ...
        if ($header === '' && $allowBodyIdempotencyKey) {
            $body = $this->payload($request);
            $header = trim((string) ($body['idempotency_key'] ?? ''));
        }
