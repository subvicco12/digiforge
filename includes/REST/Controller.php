...
use DigiForge\Launch\GelatoActivationPreflight;
use DigiForge\Launch\EtsyPublishActivationPreflight;
use DigiForge\Launch\OrderAutomationActivationPreflight;
use DigiForge\Launch\GstAutomationActivationPreflight;
...
final class Controller {
    ...
    public function routes(): void {
        ...
        register_rest_route('digiforge/v1', '/activations/gelato', ['methods' => 'POST', 'callback' => [$this, 'activate_gelato'], 'permission_callback' => [$this, 'can_manage']]);
        register_rest_route('digiforge/v1', '/activations/etsy-publish', ['methods' => 'POST', 'callback' => [$this, 'activate_etsy_publish'], 'permission_callback' => [$this, 'can_manage']]);
        register_rest_route('digiforge/v1', '/activations/order-automation', ['methods' => 'POST', 'callback' => [$this, 'activate_order_automation'], 'permission_callback' => [$this, 'can_manage']]);
        register_rest_route('digiforge/v1', '/activations/gst-automation', ['methods' => 'POST', 'callback' => [$this, 'activate_gst_automation'], 'permission_callback' => [$this, 'can_manage']]);
    }
    ...
    public function activations(\WP_REST_Request $request): \WP_REST_Response {
        return new \WP_REST_Response([
            ...
            'gelato' => (new GelatoActivationPreflight())->report(),
            'etsy_publish' => (new EtsyPublishActivationPreflight())->report(),
            'order_automation' => (new OrderAutomationActivationPreflight())->report(),
            'gst_automation' => (new GstAutomationActivationPreflight())->report(),
    ...
    public function activate_gelato(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('gelato', new GelatoActivationPreflight(), 'READY_FOR_CONTROLLED_GELATO_ACTIVATION', 'activateGelato');
    }
    public function activate_etsy_publish(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('etsy_publish', new EtsyPublishActivationPreflight(), 'READY_FOR_CONTROLLED_ETSY_PUBLISH_ACTIVATION', 'activateEtsyPublish');
    }
    public function activate_order_automation(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('order_automation', new OrderAutomationActivationPreflight(), 'READY_FOR_CONTROLLED_ORDER_AUTOMATION_ACTIVATION', 'activateOrderAutomation');
    }
    public function activate_gst_automation(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('gst_automation', new GstAutomationActivationPreflight(), 'READY_FOR_CONTROLLED_GST_AUTOMATION_ACTIVATION', 'activateGstAutomation');
    }
    private function activateScoped(string $capability, object $preflight, string $readyStatus, string $method): \WP_REST_Response|\WP_Error {
        $report = $preflight->report();
        if (($report['status'] ?? '') !== $readyStatus) {
            return new \WP_Error('digiforge_' . $capability . '_activation_blocked', __('Capability activation preflight is blocked.', 'digiforge'), ['status' => 409, 'blockers' => $report['blockers'] ?? []]);
        }
        if (! Settings::$method()) {
