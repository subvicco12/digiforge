...
final class Controller {
    ...
    public function update_control(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        ...
        $enabled = rest_sanitize_boolean($request->get_param('enabled')); if ($key !== 'stop_all' && $enabled && Settings::get('activation_authorized', false) !== true) { return new \WP_Error('digiforge_activation_not_authorized', __('External controls cannot be enabled before activation is explicitly authorized.', 'digiforge'), ['status' => 409]); } if ($key === 'stop_all' && ! $enabled && Settings::get('activation_authorized', false) !== true) { return new \WP_Error('digiforge_activation_not_authorized', __('STOP ALL cannot be disabled before activation is explicitly authorized.', 'digiforge'), ['status' => 409]); } if (! Settings::set($key, $enabled, 'boolean')) { return new \WP_Error('digiforge_control_save_failed', __('Unable to save control.', 'digiforge'), ['status' => 500]); }
