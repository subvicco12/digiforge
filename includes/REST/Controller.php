...
final class Controller {
    ...
    public function activate_etsy_publish(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('etsy_publish', new EtsyPublishActivationPreflight(), 'READY_FOR_CONTROLLED_ETSY_PUBLISH_ACTIVATION', static fn(): bool => Settings::activateEtsyPublish());
    }
    public function activate_order_automation(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('order_automation', new OrderAutomationActivationPreflight(), 'READY_FOR_CONTROLLED_ORDER_AUTOMATION_ACTIVATION', static fn(): bool => Settings::activateOrderAutomation());
    }
    public function activate_gst_automation(\WP_REST_Request $request): \WP_REST_Response|\WP_Error {
        return $this->activateScoped('gst_automation', new GstAutomationActivationPreflight(), 'READY_FOR_CONTROLLED_GST_AUTOMATION_ACTIVATION', static fn(): bool => Settings::activateGstAutomation());
    }
    private function activateScoped(string $capability, object $preflightService, string $readyStatus, callable $activate): \WP_REST_Response|\WP_Error {
        $preflight = $preflightService->report();
        if (($preflight['status'] ?? '') !== $readyStatus) {
            return new \WP_Error('digiforge_' . $capability . '_activation_blocked', __('Scoped activation preflight is blocked.', 'digiforge'), ['status' => 409, 'blockers' => $preflight['blockers'] ?? []]);
        }
        if (! $activate()) {
            return new \WP_Error('digiforge_' . $capability . '_activation_failed', __('Scoped activation failed atomically.', 'digiforge'), ['status' => 500]);
        }
