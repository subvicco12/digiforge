...
final class PodController
{
    ...
    public function transition(\WP_REST_Request $r): mixed
    {
        ...
        return $this->mutate($r,'pod_state_'.sanitize_key((string)$r['entity']).'_'.(int)$r['id'].'_'.sanitize_key($state),fn()=>(new Repository())->transition((string)$r['entity'],(int)$r['id'],$state));
    }
    ...
    private function mutate(\WP_REST_Request $r,string $operation,callable $callback,int $success=200): mixed
    {
        ...
        if(!$guard->reserve($storage,$operation)) return new \WP_Error('idempotency_conflict',__('This POD mutation has already been submitted.','digiforge'),['status'=>409]);
