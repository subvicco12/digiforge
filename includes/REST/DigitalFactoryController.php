...
final class DigitalFactoryController {
    ...
    private function create(\WP_REST_Request $r,string $type): \WP_REST_Response|\WP_Error { if(strlen((string)$r->get_body())>self::MAX_BODY_BYTES){return new \WP_Error('payload_too_large',__('JSON body exceeds 64 KiB.','digiforge'),['status'=>413]);} $key=sanitize_text_field((string)($r->get_header('Idempotency-Key'))); $result=$this->repository->create($type,(array)$r->get_json_params(),$key===''?null:$key); if(is_wp_error($result)){return $result;} $replay=(bool)($result['idempotent_replay']??false); unset($result['idempotent_replay']); $response=new \WP_REST_Response($result,$replay?200:201); if($replay){$response->header('Idempotent-Replay','true');} return $response; }
