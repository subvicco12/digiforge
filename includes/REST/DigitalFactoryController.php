...
final class DigitalFactoryController {
    ...
    private function create(\WP_REST_Request $r,string $type): \WP_REST_Response|\WP_Error { $key=sanitize_text_field((string)($r->get_header('Idempotency-Key')?:$r->get_param('idempotency_key'))); $result=$this->repository->create($type,(array)$r->get_json_params(),$key===''?null:$key); if(is_wp_error($result)){return $result;} $replay=(bool)($result['idempotent_replay']??false); unset($result['idempotent_replay']); $response=new \WP_REST_Response($result,$replay?200:201); if($replay){$response->header('Idempotent-Replay','true');} return $response; }
    private function update(\WP_REST_Request $r,string $type): \WP_REST_Response|\WP_Error { $result=$this->repository->update($type,absint($r['id']),(array)$r->get_json_params()); return is_wp_error($result)?$result:new \WP_REST_Response($result,200); }
    public function transition(\WP_REST_Request $r): \WP_REST_Response|\WP_Error { $result=$this->repository->transition(absint($r['id']),(string)$r->get_param('state')); return is_wp_error($result)?$result:new \WP_REST_Response($result,200); }
}
