...
final class Settings
{
    ...
    {
        return self::activateScoped('gelato_activation_authorized', 'product_development');
    }

    public static function activateEtsyPublish(): bool
    {
        return self::activateScoped('etsy_publish_activation_authorized', 'etsy_draft');
    }

    public static function activateOrderAutomation(): bool
    {
        if (! self::is_enabled('printify') && ! self::is_enabled('gelato')) { return false; }
        return self::activateScoped('order_automation_activation_authorized', 'product_development');
    }

    public static function activateGstAutomation(): bool
    {
        return self::activateScoped('gst_automation_activation_authorized', 'order_automation');
    }

    private static function activateScoped(string $authorizationKey, string $requiredCapability): bool
    {
        global $wpdb;
        ...
        if (false === $wpdb->query('START TRANSACTION')) { return false; }
        try {
            if (! self::persist($authorizationKey, true)) { throw new \RuntimeException('Scoped activation persistence failed'); }
            if (false === $wpdb->query('COMMIT')) { throw new \RuntimeException('activation commit failed'); }
            return true;
        } catch (\Throwable $e) {
            $wpdb->query('ROLLBACK');
