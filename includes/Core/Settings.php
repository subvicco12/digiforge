...
final class Settings
{
    ...
    private static function activateScoped(string $authorizationKey, string $dependencyAuthorizationKey, string $dependencySwitch): bool
    {
        global $wpdb;
        if (self::get($dependencyAuthorizationKey, false) !== true || ! self::is_enabled($dependencySwitch)) { return false; }
        $engine = $wpdb->get_var($wpdb->prepare(
            'SELECT ENGINE FROM information_schema.TABLES WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = %s',
            Tables::settings()
        ));
